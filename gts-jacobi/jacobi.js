...
function findX(matrix, epsilon) {
  ...
  function findBetaMatrix(matrix) {
    const result = [];
    for (let i = 0; i < matrix.length; i++) {
      result.push([0]);
    }
    for (let i = 0; i < matrix.length; i++) {
      for (let j = 0; j < matrix.length; j++) {
        if (i === j) {
          ...
          result[i][0] = matrix[i][matrix.length] / matrix[i][j];
  ...
  function theMagicHappenHere(matrix, epsilon) {
    if (checkDiaDom(matrix) === "row") {
      ...
      // console.log(findNorm("column", alpha));
      const beta = multiplyMatrices(T, findMatrixb(matrix));
...
console.log(findX(a, 0.00000001));
